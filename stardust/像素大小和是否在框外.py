...
def func():
    path = '/home/hsc/下载/50试标_501_1537357865.csv'
    with open(path, 'r', newline='') as file:
        ...
        for ind, line in enumerate(csvreader):
            ...
            for point_list in eval(line[-1]):
                ...
                with open('result/'+re.sub(r'(?<=_0)_\d{10}', '', '.'.join(url.split('/')[-1].split('.')[:-1])) + '.txt', newline='', mode='a') as file:
                    file.write(str(z) + '\t')
                    file.write(str((final_points[0][0] + final_points[1][0])/2/size[0]) + '\t')
                    file.write(str((final_points[0][1] + final_points[1][1])/2/size[1]) + '\t')
                    file.write(str((kuan)/size[0]) + '\t')
                    file.write(str((gao)/size[1]) + '\t')
                    file.write('\n')

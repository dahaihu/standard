...
# import pymysql
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:669193@localhost/first_flask"
...
if __name__ == '__main__':
    ...
    db.session.commit()
    # users = User.query.filter(not_(User.id in {1, 2})).all()
    # for user in users:
    #     print(user.id)
    # print("*"*10)
    # users = User.query.filter(User.id in {1, 2}).all()
    # for user in users:
    #     print(user.id)
